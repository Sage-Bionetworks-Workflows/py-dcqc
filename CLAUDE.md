...
- **`docs/`** — Sphinx site. `docs/api/` is deleted and regenerated by `docs/conf.py:36-53` on *every* build, so never hand-edit it. `docs/readme.md`, `contributing.md`, `authors.md`, `changelog.md` are MyST `{include}` shims — edit the corresponding file at the repo root instead. `CHANGELOG.md` holds no entries on purpose — it points at the GitHub releases page, which is where release notes live.
