...
See README.md for concepts and CLI usage, CONTRIBUTING.md for setup, ARCHITECTURE.md for adding a new file type, suite, or test.
...
- **`docs/`** — Sphinx site. `docs/api/` is deleted and regenerated by `docs/conf.py:36-53` on *every* build, so never hand-edit it. `docs/readme.md`, `contributing.md`, `architecture.md`, `authors.md`, `changelog.md` are MyST `{include}` shims — edit the corresponding file at the repo root instead. `docs/index.md` and `CHANGELOG.md` are still untouched PyScaffold placeholders.
...
- **Do NOT reword a `status_reason` string without grepping the tests** — several are asserted verbatim, for example `tests/test_internal_tests.py:178` pins "FASTQ files do not have the same number of lines".
