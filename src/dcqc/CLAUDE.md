...
- **`FileType("*", (), ...)`** has an empty extension tuple, and `str.endswith(())` is always `False`. Any file without a `file_type` falls back to `"*"`, lands in `FileSuite`, and therefore **always fails** `FileExtensionTest`. An empty tuple is a valid `Collection`, so it passes `_validate_file_extensions` deliberately — do not make that guard reject an empty collection to work around this.
